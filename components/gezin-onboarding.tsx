...
export function GezinOnboarding({ initialCode = "" }: { initialCode?: string }) {
  ...
  async function submitCreate(event: React.FormEvent) { event.preventDefault(); setBezig(true); setFout(null); try { const r = await maakGezinAan(naam); setResultaat(r.code) } catch (e) { setFout(e instanceof Error ? e.message : "Er ging iets mis.") } finally { setBezig(false) } }
